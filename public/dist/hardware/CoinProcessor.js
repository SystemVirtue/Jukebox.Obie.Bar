...
(function (SerialErrors) {
    ...
    SerialErrors["InvalidCode"] = "INVALID_CODE";
    SerialErrors["DeviceNotFound"] = "DEVICE_NOT_FOUND";
    SerialErrors["ConnectionFailed"] = "CONNECTION_FAILED";
...
export class CoinProcessor {
    ...
    async connect() {
        try {
            ...
            this.handleError(SerialErrors.ConnectionFailed, `Failed to connect to coin acceptor: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
    ...
    async startListening() {
        if (!this.port || !this.port.readable || this.isListening)
            return;
        ...
        try {
            ...
            this.handleError(SerialErrors.ConnectionFailed, `Error reading from serial port: ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
            this.isListening = false;
     ...
     */
    showCreditAddedFeedback(amount) {
