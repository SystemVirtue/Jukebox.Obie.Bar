...
document.addEventListener('DOMContentLoaded', () => {
    ...
    playerButton.id = 'player-control';
    playerButton.innerHTML = 'Open Player Window';
    playerButton.style.position = 'fixed';
    playerButton.style.bottom = '20px';
    playerButton.style.right = '20px';
    playerButton.style.padding = '10px 15px';
    playerButton.style.backgroundColor = '#ff0000';
    playerButton.style.color = 'white';
    playerButton.style.border = 'none';
    playerButton.style.borderRadius = '4px';
    playerButton.style.cursor = 'pointer';
    playerButton.style.zIndex = '1000';
    playerButton.addEventListener('click', () => playerService.openPlayer());
    document.body.appendChild(playerButton);
    
    // Update button text based on player state
    playerService.addStatusListener((status) => {
        if (status.type === 'playerClosed') {
            playerButton.textContent = 'Open Player Window';
        } else if (status.type === 'stateChange' && status.state === YT.PlayerState.PLAYING) {
            playerButton.textContent = 'Player: Playing';
        } else if (status.type === 'stateChange' && status.state === YT.PlayerState.PAUSED) {
            playerButton.textContent = 'Player: Paused';
        }
    });
    ...
    // Implement search dialog functionality
    const createSearchDialog = () => {
        ...
        const displaySearchResults = (results: SearchResult[]) => {
            if (results.length === 0) {
    ...
    // Register the search function globally so it can be called from jukebox.html
    window.openSearchDialog = createSearchDialog;
    
    // Listen for the custom event from jukebox.html
    ...
    // Add keyboard shortcut for player (Ctrl+P or Cmd+P)
...
// Add some basic styles for the player control button
